...
def ball_callback(data):
    ball_position.x = data.x
    ball_position.y = data.y
    ball_position.z = data.z

def get_trajectory(fa, target):
    cur_pose = fa.get_pose()
    target_pose = RigidTransform(translation=np.array([target.x, target.y, target.z]))
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("ball", Point, ball_callback)
    ...
    fa.goto_pose(pose_traj[1], duration=T, dynamic=True, buffer_time=30,
